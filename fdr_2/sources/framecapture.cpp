...
void FrameCapture::visualize(const cv::Mat& frame, const std::vector<cv::Rect>& faces, const std::vector<std::string>& names) {
    cv::Mat vis = frame.clone();
    for (size_t i = 0; i < faces.size(); ++i) {
        cv::rectangle(vis, faces[i], cv::Scalar(0, 255, 0), 2);
        if (i < names.size())
            cv::putText(vis, names[i], {faces[i].x, faces[i].y - 10}, cv::FONT_HERSHEY_SIMPLEX, 0.8, {255, 0, 0}, 2);
    }
